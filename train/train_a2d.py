...
from datasets.a2d import A2DDataset
...
def train():
    ...
    start_epoch = 1
    load_path, client_state = model.load_checkpoint(load_dir='weights/checkPoint_full_video_mamba',
    ...
    for epoch in range(start_epoch, max_epochs + 1):
        ...
        for iter, data in tqdm(enumerate(dataset_loader, 1), total=len(dataset_loader), desc=f'Epoch {epoch}/{max_epochs}'):
            ...
            # use only first frame to match downstream shapes
            # imgs_sam = imgs_sam[:, 0, :, :, :].unsqueeze(1).to(dtype=model.dtype, device=model.device)
            # imgs_beit = imgs_beit[:, 0, :, :, :].unsqueeze(1).to(dtype=model.dtype, device=model.device)
            ...
            masks = masks.to(dtype=model.dtype, device=model.device)

            # loss
            optimizer.zero_grad()
            # loss = model(imgs_sam, imgs_beit, input_ids, torch.ones_like(input_ids), None, masks, size_tuple, resize)
            loss = model(imgs_sam, imgs_beit, input_ids, torch.ones_like(input_ids), None, masks, size_tuple, resize,
                         num_frames=T, sampled_indices=sampled_indices)
        ...
        model.save_checkpoint(save_dir='weights/checkPoint_full_video_mamba', tag=f'epoch_{epoch:04d}',
