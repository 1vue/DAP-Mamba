...
from utils import load_new_model
from transformers import AutoConfig
warnings.filterwarnings('ignore')
import alphaclip
from alphaclip import alpha_clip
def printLayers(new_layer_params,old_layer_params,captured_names):
    new_param_count = sum(param.numel() for param in new_layer_params)
    old_param_count = sum(param.numel() for param in old_layer_params)
    total_param_count = new_param_count + old_param_count

    print(f"\n📊 Parameter group summary:")
    print(f"   🔥 New layers (High LR 5e-5): {len(new_layer_params)}  tensors")
    print(f"   🔥 New parameter count: {new_param_count:,} ({new_param_count / 1e6:.3f}M)")
    print(f"   ❄️ Old layers (Low LR 5e-6):  {len(old_layer_params)}  tensors")
    print(f"   ❄️ Old layers: {old_param_count:,} ({old_param_count / 1e6:.3f}M)")
    print(f"   📦 Total parameter count: {total_param_count:,} ({total_param_count / 1e6:.3f}M)")
    if total_param_count > 0:
        print(f"   📈 New parameter ratio: {new_param_count / total_param_count * 100:.3f}%")
...
def getlayers(rank,model):
    ...
    old_layer_params = []
    target_keywords = [
        'temporal_fusion','prompt_interaction','weight_proj','temporal_pe'
    ]
    captured_names = []
...
def init_models():
    ...
    # dap_mamba = DAPMambaModel.from_pretrained(model_path, low_cpu_mem_usage=False, strict=False, local_files_only=True)
    ...
    evfsam = evfsam.cuda()
    evfsam.train()
    check_initialization(evfsam)
    print("DAP-Mamba model initialization complete!")
...
def train():
    ...
    # base_optimizer = optim.AdamW(model.parameters(), lr=5e-5)
    ...
    if len(new_layer_params)==0:
        ...
        base_optimizer = optim.AdamW([
            {'params': new_layer_params, 'lr': 7e-6},
            {'params': old_layer_params, 'lr': 5e-6},
        ])
    max_epochs = 10
    ...
    start_epoch = 1
    # load_path, client_state = model.load_checkpoint(load_dir='weights/checkPoint_full_video_mamba', load_module_strict=False)
    # training loop

    # if load_path is not None:
    # else:

    scheduler = CosineAnnealingLR(base_optimizer, T_max=max_epochs, eta_min=1e-6,last_epoch=start_epoch-2)
    for epoch in range(start_epoch, max_epochs + 1):
        ...
        for iter, data in tqdm(enumerate(dataset_loader, 1), total=len(dataset_loader), desc=f'Epoch {epoch}/{max_epochs}'):
            ...
            if math.isnan(current_loss) or math.isinf(current_loss):
                print(f"❌ [Error] Iter {iter}: Loss is NaN or Inf!")
                print(f"❌ [Error] Iter {iter}: Loss is NaN or Inf!")
                print(f"    - BCE: {loss['mask_bce_loss'].item()}")
                print(f"    - Dice: {loss['mask_dice_loss'].item()}")
                optimizer.zero_grad()
                continue
        ...
        model.save_checkpoint(save_dir='weights/checkPoint_full_video_mamba', tag=f'epoch_{epoch:04d}',client_state=client_state)
        if rank == 0:
            print(f"Checkpoint saved: weights/checkPoint_full_video_mamba/epoch_{epoch:04d}")
            save_path = os.path.join('weights/checkPoint_full_video_mamba/video_mamba{:04d}.pth'.format(epoch))
        ...
        #     save_path = os.path.join('weights/hq_sam_{:04d}.pth'.format(epoch))
